...
class AccessTokenWrapper
  ...
  def refresh!
    ...
    $redis.set( @@key, access_token.to_hash.to_json )
  end
...
def get_upload_token(file_path, access_token)
  raise "#{file_path} is not a valid file path" unless File.file?(file_path)
  ...
  upload_token = `#{command}`

  puts upload_token

  raise "authentication invalid #{upload_token}" if upload_token.include? 'Authentication session is not defined.'
  raise(InsufficientQuota, 'quota exeeded') if upload_token.include? 'Insufficient tokens for quota'
  raise notify "upload token was not retrieved by google photos api" unless upload_token
...
def upload_to_google_photo(file_path, description, is_delete=true)
  ...
  update_recording_job(uuid_from_file_name(file_path), 'UPLOAD')
end

def get_delay_time(consecutive_error_count)
  consecutive_error_count ** 5 + 15
end
